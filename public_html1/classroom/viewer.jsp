...
//정보-사이트설정
DataSet siteconfig = SiteConfig.getArr(new String[] {"lesson_", "kt_remote", "lanedu_type"});
...
//랜선에듀biz의 경우 첫 입장시 참석자 등록필요, 이후 토큰만 재발행
if("15".equals(info.s("lesson_type")) && "".equals(info.s("p_reg_date"))) { info.put("init_lanedu", "Y"); }
...
if("01".equals(info.s("lesson_type")) || "03".equals(info.s("lesson_type"))) {
	...
	if("B".equals(siteconfig.s("lanedu_type"))) {
		...
		//첫 입장시 회의참석자 등록
		if(info.b("init_lanedu")) { lanEdu.insertMember(info.s("twoway_url"), cuid, loginId, "USER", ""); }

		Json j = lanEdu.getPlanUrl(info.s("twoway_url"), cuid, "USER");
		...
		startUrl = ktRemote.getPlanUrl(info.s("twoway_url"), cuid, loginId, "USER", "");
	}
	info.put("start_url", startUrl);
	info.put("start_url_conv", startUrl);
}
