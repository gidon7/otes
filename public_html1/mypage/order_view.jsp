...
if("del".equals(m.rs("mode"))) {
	...
	if(2 == info.i("status") && ("03".equals(info.s("paymethod")) || "90".equals(info.s("paymethod")))) {
		...
		freepassUser.item("status", -4);
		freepassUser.update("order_id = " + id);
...
DataSet list = orderItem.query(
	...
	+ " LEFT JOIN " + refund.table + " r ON a.product_type = 'course' AND a.id = r.order_item_id AND r.status != -2"
	+ " LEFT JOIN " + order.table + " o ON a.order_id = o.id AND o.status != -1"
