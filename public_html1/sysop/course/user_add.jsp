...
if("check".equals(m.rs("mode"))){
	//int cuinfo = courseUser.findCount("site_id = " + siteId + " AND status != -1 AND course_id = " + courseId + " AND user_id IN (" + m.rs("uid") + ") ");
	DataSet cuinfo = courseUser.query("SELECT DISTINCT user_id FROM " + courseUser.table + " WHERE site_id = " + siteId + " AND status != -1 AND course_id = " + courseId + " AND user_id IN (" + m.rs("uid") + ") ");

	JSONObject json = new JSONObject();
	JspWriter jout = (JspWriter) out;
	json.put("cucnt", cuinfo.size());
	json.put("cid",courseId );
	json.put("uid",m.rs("uid") );
