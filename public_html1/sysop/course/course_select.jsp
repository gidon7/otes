...
//일괄등록
if("add".equals(f.get("mode2")) && !"".equals(f.get("crid"))) {

	int aid = m.ri("aid");
	if(aid == 0) { m.jsError("기본키는 반드시 지정해야 합니다."); return; }

	String[] idx = f.get("crid").split("\\,");
	for(int i=0; i<idx.length; i++) {
		if(!courseAuto.add(aid, m.parseInt(idx[i]))) { m.jsError("등록하는 중 오류가 발생했습니다."); return; }
	}
	
	m.jsAlert("추가 되었습니다.");
	out.print("<script> try { parent.opener.location.reload(); } catch (e) { } </script>");
	m.jsReplace("course_select.jsp?mode=" + f.get("mode") + "&aid=" + aid, "parent") ;
	return;
}
...
if("auto".equals(mode)) lm.addWhere("NOT EXISTS (SELECT 1 FROM " + courseAuto.table + " ca WHERE ca.course_id = a.id AND ca.auto_id = " + m.ri("aid") + " AND ca.site_id = " + siteId + ")");
