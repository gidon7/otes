...
if("check".equals(m.rs("mode"))){
	String[] idx = m.rs("ord").split("\\,");
	String[] uidArr1 = m.rs("uid").split("\\,");
	String[] uidArr2 = new String[idx.length];

	for(int i = 0; i < idx.length; i++) {
		int index = m.parseInt(idx[i]) - 1;
		uidArr2[i] = uidArr1[index];
	}

	String uid = Arrays.toString(uidArr2).substring(1,Arrays.toString(uidArr2).length()-1);

	DataSet cuinfo = courseUser.query("SELECT DISTINCT user_id FROM " + courseUser.table + " WHERE site_id = " + siteId + " AND status != -1 AND course_id = " + courseId + " AND user_id IN (" + uid + ") ");


	JSONObject json = new JSONObject();
	JspWriter jout = (JspWriter) out;
	json.put("cucnt", cuinfo.size());
	json.put("uid", uid);
	jout.write(json.toString());
	return;
}
