...
//사용횟수재산정
if("updateCnt".equals(m.rs("mode"))){
	int userCnt = freepassUser.updateCount(fuid);
	m.jsAlert("재산정하였습니다.");
	m.jsReplace("freepass_view.jsp?" + m.qs("mode"));
}
...
if(m.isPost() && f.validate()) {
	...
	m.jsAlert("수정하였습니다.");
	if(!ch.equals("poplayer")) m.jsReplace("freepass_view.jsp?" + m.qs());
	else m.js("parent.document.location.reload()");
	return;
}
...
//	+ " INNER JOIN " + courseUser.table + " cu ON a.id = cu.order_item_id "
	...
	+ " WHERE a.freepass_user_id = " + fuid + " AND a.status IN (1,2,10,20,-2) "
