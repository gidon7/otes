...
if("del".equals(m.rs("mode"))) {
	...
	//정보-before
	DataSet binfo = refund.query(
			" SELECT a.*, "
			+ " o.refund_price o_refund_price, o.refund_date o_refund_date, o.status o_status, "
			+ " oi.refund_price oi_refund_price, oi.refund_date oi_refund_date, oi.status oi_status "
			+ " FROM " + refund.table + " a "
			+ " INNER JOIN " + order.table + " o ON a.order_id = o.id AND o.site_id = " + siteId + ""
			+ " INNER JOIN " + orderItem.table + " oi ON a.order_item_id = oi.id AND oi.site_id = " + siteId + ""
			+ " WHERE a.id = ? AND a.site_id = " + siteId
			, new Object[] {id}
	);
	if(!binfo.next()) { m.jsError("해당 정보가 없습니다. before"); return; }
	...
	if(!ainfo.next()) { m.jsError("해당 정보가 없습니다. after"); return; }
	...
	actionLog.item("action_desc", "환불내역삭제");
