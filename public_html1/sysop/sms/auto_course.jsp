...
if("add".equals(f.get("mode"))) {
	...
	if(!courseAuto.add(aid, crid)) { m.jsError("등록하는 중 오류가 발생했습니다."); return; }
	...
	m.jsReplace("auto_course.jsp?aid=" + aid);
	return;
} else if("mdel".equals(f.get("mode"))){
	//기본키
	String crid = f.get("crid");
	if("".equals(crid)) { m.jsError("기본키는 반드시 지정해야 합니다."); return; }

	//정보
	DataSet crinfo = course.find("id IN (" + crid + ") AND site_id = " + siteId + " AND status != -1");
	if(!crinfo.next()) { m.jsError("해당 과정정보가 없습니다."); return; }
	crinfo.first();
	while (crinfo.next()){
		if(!courseAuto.del(aid, crinfo.i("id"))) {m.jsError("삭제하는 중 오류가 발생했습니다."); return; }
	}
